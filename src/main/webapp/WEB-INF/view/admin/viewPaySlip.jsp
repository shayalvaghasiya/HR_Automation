...
<head>
  ...
  <link rel="stylesheet" href="<%=request.getContextPath()%>/adminResource/css/dataTables.bootstrap4.css" />
...
<body>
          ...
          <div class="tab-content" id="setting-content">
            ...
            <div class="tab-pane fade" id="chats-section" role="tabpanel" aria-labelledby="chats-section">
              ...
              <ul class="chat-list">
                ...
                <li class="list">
                  <div class="profile"><img src="<%=request.getContextPath()%>/adminResource/images/face25.jpg" alt="image"><span class="online"></span></div>
                  <div class="info">
                ...
                <li class="list">
                  <div class="profile"><img src="<%=request.getContextPath()%>/adminResource/images/face12.jpg" alt="image"><span class="online"></span></div>
                  <div class="info">
                    <p>Sarah Graves</p>
                    <p>Available</p>
                  </div>
                    ...
                    <table id="order-listing" class="table">
                      ...
                      <tbody>
                       <%@taglib prefix="c" uri="http://java.sun.com/jstl/core_rt" %>
                        ...
                        <c:forEach items="${attendanceList}" var="i" varStatus="j">

	                        <c:forEach items="${employeeList}" var="k">	

                            		<c:if test="${i.employeeIDno eq k.employeeIDno }">
				                        <tr>
        			                    	<td>${j.count}</td>
                    			        	<td>${i.attendanceFileMonth}</td>
                            				<td>${i.employeeIDno}</td>
		                            		<td>
	    	                        			${k.employeeFirstName} ${k.employeeLastName}
	        	                    		</td>
                	            			<td>
                	            			
                	            				<c:if test="${k.paySlipStatus eq 'pending'}">
                    	          					<button class="btn btn-outline-primary"><a href="generatePaySlip.html?employeeId=${k.id}">Generate Pay Slip</a></button>
                        	    				</c:if>
 												<c:if test="${k.paySlipStatus ne 'pending'}">
                    	          					${k.paySlipStatus}
                        	    				</c:if>
                        	    			</td>
                            				 <td>                        
                            				 	<c:if test="${k.paySlipStatus eq 'Generated'}">
                            				 		<c:forEach items="${salarySlipList}" var="l">
                            				 			<c:if test="${l.employeeVO.id eq k.id}">
															<button class="btn btn-outline-primary"><a href="${l.salarySlipLink}">DOWNLOAD</a></button>
														</c:if>
													</c:forEach>
                            					</c:if>
												<c:if test="${k.paySlipStatus ne 'Generated'}">
                            						${k.paySlipStatus}
                            					</c:if>
                            		...
                            		<c:if test="${i.employeeIDno ne k.employeeIDno }">
                            		</c:if>
                        	</c:forEach>	
  ...
  <script src="<%=request.getContextPath()%>/adminResource/js/jquery.dataTables.js"></script>
  <script src="<%=request.getContextPath()%>/adminResource/js/dataTables.bootstrap4.js"></script>
  ...
  <script src="<%=request.getContextPath()%>/adminResource/js/data-table.js"></script>
